...
static void handle_keys(t_data *data)
{
	...
	if (data->move_up) {
		double nextPlayerX = data->playerX + cos(data->playerDir) * 0.1;
		double nextPlayerY = data->playerY + sin(data->playerDir) * 0.1;
...
void draw_walls(t_data *data)
{
	...
	{	
		for (int i = 0; i < 5; i++) {
			double cameraX = (2 * x + i - 2) / (double)(SCREENSIZE * 2) - 1;
			double rayDirX = cos(data->playerDir + cameraX * data->fov);
			double rayDirY = sin(data->playerDir + cameraX * data->fov);
